...
def fetch_product_id (product_name, country, retries=10) :
	try :
		request = requests.get(
			'https://europe-west1-panprices.cloudfunctions.net/pricerunner_products?' +
			'product_name=' + urlify(product_name) + 
			'&country=' + country
		)
	except requests.exceptions.Timeout :
		print "Got a timeout on product " + product_name + " on Pricerunner."
...
def fetch_product_offers(product_url, country, retries=10) :
	try :
		request = requests.get(
			'https://europe-west1-panprices.cloudfunctions.net/pricerunner_offers?' +
